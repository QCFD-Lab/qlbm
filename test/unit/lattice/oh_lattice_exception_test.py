...
def test_lattice_exception_mismatched_velocities_and_dims():
    with pytest.raises(LatticeException) as excinfo:
        OHLattice({"lattice": {"dim": {"x": 64}, "velocities": "D2Q4"}})
...
def test_lattice_exception_unsupported_discretization():
    with pytest.raises(LatticeException) as excinfo:
        OHLattice({"lattice": {"dim": {"x": 64}, "velocities": {"x": 4}}})
...
def test_lattice_exception_marker_index():
    lattice = OHLattice({"lattice": {"dim": {"x": 64, "y": 64}, "velocities": "D2Q9"}})
    with pytest.raises(LatticeException) as excinfo:
        lattice.marker_index()

    assert "Multiple geometries not yet supported for OHLattice." == str(excinfo.value)


def test_lattice_exception_accumulation_index():
    lattice = OHLattice({"lattice": {"dim": {"x": 64, "y": 64}, "velocities": "D2Q9"}})
    with pytest.raises(LatticeException) as excinfo:
        lattice.accumulation_index()

    assert "Accumulation not yet supported for OHLattice." == str(excinfo.value)
